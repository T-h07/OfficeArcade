import { useCallback, useEffect, useState } from "react";
import {
  equipInventoryItem,
  getStoreCatalog,
  getStoreSummary,
  purchaseStoreItem,
  StoreApiError,
  unequipInventoryItem
} from "../api/storeApi";
import type {
  CosmeticCategory,
  CosmeticRarity,
  StoreCatalogResponse,
  StoreSummaryResponse
} from "../types/store.types";

type UseStoreResult = {
  summary: StoreSummaryResponse | null;
  catalog: StoreCatalogResponse | null;
  categoryFilter: CosmeticCategory | "ALL";
  rarityFilter: CosmeticRarity | "ALL";
  isLoading: boolean;
  isMutating: boolean;
  errorMessage: string | null;
  actionMessage: string | null;
  setCategoryFilter: (value: CosmeticCategory | "ALL") => void;
  setRarityFilter: (value: CosmeticRarity | "ALL") => void;
  refresh: () => Promise<void>;
  purchase: (itemId: string) => Promise<boolean>;
  equip: (itemId: string) => Promise<boolean>;
  unequip: (itemId: string) => Promise<boolean>;
  clearActionMessage: () => void;
};

function resolveErrorMessage(error: unknown, fallbackMessage: string) {
  if (error instanceof Error && error.message.trim().length > 0) {
    return error.message;
  }
  return fallbackMessage;
}

export function useStore(accessToken: string | null, onUnauthorized: () => void): UseStoreResult {
  const [summary, setSummary] = useState<StoreSummaryResponse | null>(null);
  const [catalog, setCatalog] = useState<StoreCatalogResponse | null>(null);
  const [categoryFilter, setCategoryFilter] = useState<CosmeticCategory | "ALL">("ALL");
  const [rarityFilter, setRarityFilter] = useState<CosmeticRarity | "ALL">("ALL");
  const [isLoading, setIsLoading] = useState(true);
  const [isMutating, setIsMutating] = useState(false);
  const [errorMessage, setErrorMessage] = useState<string | null>(null);
  const [actionMessage, setActionMessage] = useState<string | null>(null);

  const refresh = useCallback(async () => {
    if (!accessToken) {
      setSummary(null);
      setCatalog(null);
      setErrorMessage(null);
      setIsLoading(false);
      return;
    }

    setIsLoading(true);
    setErrorMessage(null);

    try {
      const [nextSummary, nextCatalog] = await Promise.all([
        getStoreSummary(accessToken),
        getStoreCatalog(
          accessToken,
          categoryFilter === "ALL" ? undefined : categoryFilter,
          rarityFilter === "ALL" ? undefined : rarityFilter
        )
      ]);
      setSummary(nextSummary);
      setCatalog(nextCatalog);
    } catch (error) {
      if (error instanceof StoreApiError && error.status === 401) {
        onUnauthorized();
        return;
      }
      setErrorMessage(resolveErrorMessage(error, "Unable to load store data."));
    } finally {
      setIsLoading(false);
    }
  }, [accessToken, categoryFilter, rarityFilter, onUnauthorized]);

  useEffect(() => {
    void refresh();
  }, [refresh]);

  async function runMutation(
    action: () => Promise<unknown>,
    successMessage: string,
    fallbackErrorMessage: string
  ): Promise<boolean> {
    if (!accessToken) {
      return false;
    }
    setIsMutating(true);
    setErrorMessage(null);
    setActionMessage(null);

    try {
      await action();
      setActionMessage(successMessage);
      await refresh();
      return true;
    } catch (error) {
      if (error instanceof StoreApiError && error.status === 401) {
        onUnauthorized();
        return false;
      }
      setErrorMessage(resolveErrorMessage(error, fallbackErrorMessage));
      return false;
    } finally {
      setIsMutating(false);
    }
  }

  return {
    summary,
    catalog,
    categoryFilter,
    rarityFilter,
    isLoading,
    isMutating,
    errorMessage,
    actionMessage,
    setCategoryFilter,
    setRarityFilter,
    refresh,
    purchase: (itemId) =>
      runMutation(
        () => purchaseStoreItem(accessToken!, itemId),
        "Purchase completed.",
        "Unable to purchase selected item."
      ),
    equip: (itemId) =>
      runMutation(
        () => equipInventoryItem(accessToken!, itemId),
        "Item equipped.",
        "Unable to equip selected item."
      ),
    unequip: (itemId) =>
      runMutation(
        () => unequipInventoryItem(accessToken!, itemId),
        "Item unequipped.",
        "Unable to unequip selected item."
      ),
    clearActionMessage: () => setActionMessage(null)
  };
}
